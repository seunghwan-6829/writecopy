...
export async function POST(request: NextRequest) {
  ...
  if (!originalContent) {
    return new Response(
      JSON.stringify({ success: false, error: "원본 자기소개서가 없습니다." }),
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      // 해당 모델로 6개 베리에이션 생성
      const isGPT = model === "GPT-5.2";
      
      const tasks = Array.from({ length: 6 }, (_, i) => ({
        id: i + 1,
        model: isGPT ? "GPT-5.2" : "Claude 4.5 Sonnet",
        versionNum: i + 1,
      }));
            ...
            if (isGPT) {
              ...
              const response = await openai.chat.completions.create({
                ...
                messages: [
                  ...
                  {
                    role: "user",
                    content: `베리에이션 ${task.versionNum}번을 작성해주세요. 이전 버전들과 다른 관점이나 강조점으로 작성해주세요.`,
                  },
                ],
